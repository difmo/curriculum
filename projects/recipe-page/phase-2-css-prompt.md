...
Note: There used to be a "Bootstrap" part of this project, but Bootstrap is no longer a part of the Difmo curriculum. If you'd like to research and incorporate a library to streamline the CSS process, you can do that on your own.
...
1. On your desktop, create a folder called `Difmo-projects`. Inside that, create a folder called `recipe-page`. Move your `recipe-page.html` file into `Difmo-projects/recipe-page`. If you no longer have `recipe-page.html`, make it again with the Challenge directions from [Phase 1](./phase-1-html-prompt.md#challenge).
2. Use TextEdit or WordPad to make a new file called `styles.css`. Make sure it is in the `Difmo-projects/recipe-page` directory right next to `recipe-page.html`.
3. Now that it's in `Difmo-projects/recipe-page`, it's obvious what your HTML file is for. Rename `recipe-page.html` to `index.html`.
