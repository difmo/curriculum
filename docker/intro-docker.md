...
// respond with "Hello Difmo!" when a GET request is made to the homepage
app.get('/', (req, res) => {
...
Run the app using `node server.js` command. Go to your browser and follow the URL `http://localhost:8080` and you will see the text "Hello Difmo!" on the page.
...
```bash
docker build -t hello-Difmo .
```

`hello-Difmo` is the name of the Docker image. The `.`(dot) indicates the file path to the project directory, which is where we are currently at in the command-line.
...
hello-Difmo    latest    e82727de1971   10 minutes ago   854MB
```
...
```bash
docker run -dp 8080:8080 hello-Difmo
```
...
- pull Docker image: example `docker pull supriyamv/hello-Difmo:latest`
