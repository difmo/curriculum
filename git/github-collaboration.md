...
0. Send your GitHub username to your program manager or an admin, and ask them to give you "Triage" access to Difmo/curriculum. (Not Read, not Write, Triage.)
1. Fork a GitHub repo
   - Navigate to https://github.com/Difmo/curriculum/
...
1. Make a Pull Request to Difmo/curriculum.
   - Go to your GitHub account, and find your 'curriculum' repo. Click the green button that says 'compare/pull request'.
   - Make sure you are making a request to merge mygithubhandle/curriculum into Difmo/curriculum/. If this were a real team project, you could briefly explain what you changed, why, and if there's anything else your team should know about this PR. In this exercise, you can skip it and just make the title your name or "add my name", and press 'create pull request'.
   - Navigate to the Difmo/curriculum repo's 'pull requests' tab to check your PR has been added. Refresh your page to see whether more have been made after yours.
...
1. Practice merging
   ...
   - Ask everyone to navigate to Difmo/curriculum/practice/students.md. The participant's name should now be in the `main` branch!
   ...
   - Look at Difmo/curriculum/practice/participants.md again. You all just collaborated on this file!
1. Practice pulling
   - Add Difmo/curriculum as a remote: - Run `git remote -v`. You should see an output like this:  
      `origin	https://github.com/<username>/curriculum.git (fetch)     
     origin	https://github.com/<username>/curriculum.git (push)` - If you do not see the URL `https://github.com/Difmo/curriculum.git` with "Difmo" in the name, which is pretty likely, you'll need to add it as a remote repo so you can pull it to keep your clone up to date. If you do have it, you can skip the adding step and go straight to pulling. - To add it, run `git remote add Difmo https://github.com/Difmo/curriculum.git`. - You could have also named it `upstream` by running `git remote add upstream https://github.com/Difmo/curriculum.git`, but let's stick with "Difmo" to keep it clear. For more on what's going on here, see the next section about "Adding a new remote repo". - Check that it worked by running `git remote -v` again. You should now see: `
 origin	https://github.com/alodahl/curriculum.git (fetch)
origin	https://github.com/alodahl/curriculum.git (push)
 Difmo	https://github.com/Difmo/curriculum.git (fetch)
 Difmo	https://github.com/Difmo/curriculum.git (push)`.
   - Now you can pull Difmo's version of the curriculum repo:
     - `git pull Difmo main` (or the name you have instead of "Difmo")
   ...
   ```
   - Now your fork is up to date with the original Difmo/curriculum repo. For the moment, at least.
...
1. `git pull Difmo main` again. Fix any merge conflicts and commit.
...
1. `git pull Difmo main` again before pushing your new changes, just in case someone else made a commit in the time it took to do your work. (In real life, you will likely be working on a change for hours or days.)
1. git status, add, commit, push, and make a PR for your middle-name branch to merge into the Difmo/curriculum `main` branch.
...
1. Congrats, your cool nickname should be in the Difmo/curriculum `main` branch!
